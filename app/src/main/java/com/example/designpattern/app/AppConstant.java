...
public class AppConstant {
    ...
    public final static String ADAPTER_DEFINE = "<em>适配器模式（Adapter Pattern）</em>将一个类的接口转换成客户期望的另一个接口，适配器让原本接口不兼容的类可以相互合作。这个定义还好，说适配器的功能就是把一个接口转成另一个接口。";
    public final static String COMMAND_DEFINE = "<em>命令模式（Command Pattern）</em>将“请求”封装成对象，以便使用不同的请求、队列或者日志来参数化其他对象。命令模式也支持可撤销的操作。(简化: <b>将请求封装成对象，将动作请求者和动作执行者解耦。</b>)";
    ...
    public final static String FACADE_DEFINE = "<em>外观模式（Facade Pattern）</em>提供一个统一的接口，用来访问子系统中的一群接口，外观定义了一个高层的接口，让子系统更容易使用。其实就是<b>为了方便客户的使用，把一群操作，封装成一个方法。</b>)";
    ...
    public final static String PROTOTYPE_DEFINE = "<em>原型模式（Prototype Pattern）</em>是用于创建重复的对象，同时又能保证性能。这种类型的设计模式属于创建型模式，它提供了一种创建对象的最佳方式。";
    ...
    public final static String MEMENTO_DEFINE = "<em>备忘录模式（Memento Pattern）</em>：保存一个对象的某个状态，以便在适当的时候恢复对象，属于行为型模式。<b>就是在不破坏封装的前提下，捕获一个对象的内部状态，并在该对象之外保存这个状态，这样可以在以后将对象恢复到原先保存的状态。</b>";
    public final static String INTERPRETER_DEFINE = "<em>解释器模式（Interpreter Pattern）</em>：提供了评估语言的语法或表达式的方式，属于行为型模式。这种模式实现了一个表达式接口，该接口解释一个特定的上下文。这种模式被用在 SQL 解析、符号处理引擎等。";
